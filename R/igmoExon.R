...
{     
   ...
   if(condition[1]=="Yes"){
    numofcondition<-max(condition_all);
    index<-1;
    celname_condition<-c();
    }else{
    numofcondition<-1;
...
for(i in c(1:numofcondition)){
    for(j in c(1:length(condition_all))){
         if(condition_all[j]==i&&condition[1]=="Yes"){
          celname_condition[index]<-celnames[j];
          index<-index+1;
         }else if(condition[1]!="Yes"){
         celname_condition<-celnames;
         break;
          }
    }
   ...
   object_condition_names<-c(object_condition_names,celname_condition); 
      ...
      if(condition[1]=="Yes"&&i==numofcondition){
...
###load corresponding between gene and transcirpt and probes and alpha num of every gene
 ...
 prctiles <- 0.01*c(5, 25, 50, 75, 95);
  ...
  if(is.null(cl)){
     ...
     expr_g_all<-rbind(expr_g_all,expr_g)
     se_g_all<-rbind(se_g_all,se_g)
     prc5_g_all<-rbind(prc5_g_all,prc5_g)
     pr25_g_all<-rbind(prc25_g_all,prc25_g)
     prc50_g_all<-rbind(prc50_g_all,prc50_g)
     prc75_g_all<-rbind(prc75_g_all,prc75_g)
     prc95_g_all<-rbind(prc95_g_all,prc95_g)

     expr_t_all<-rbind(expr_t_all,expr_t)
     se_t_all<-rbind(se_t_all,se_t)
     prc5_t_all<-rbind(prc5_t_all,prc5_t)
     pr25_t_all<-rbind(prc25_t_all,prc25_t)
     prc50_t_all<-rbind(prc50_t_all,prc50_t)
     prc75_t_all<-rbind(prc75_t_all,prc75_t)
     prc95_t_all<-rbind(prc95_t_all,prc95_t)
